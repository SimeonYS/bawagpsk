...
class BawagpskPipeline:

    # Database setup
    conn = sqlite3.connect('bawagpsk.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bawagpsk`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bawagpsk WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bawagpsk (date, title, link, content)"
